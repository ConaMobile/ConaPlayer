package com.conamobile.conaplayer.fragments.folder
...
import com.conamobile.conaplayer.adapter.SongFileAdapter
import com.conamobile.conaplayer.adapter.Storage
import com.conamobile.conaplayer.adapter.StorageAdapter
import com.conamobile.conaplayer.adapter.StorageClickListener
import com.conamobile.conaplayer.databinding.FragmentFolderBinding
...
import com.conamobile.conaplayer.helper.MusicPlayerRemote.openQueue
import com.conamobile.conaplayer.helper.menu.SongsMenuHelper
import com.conamobile.conaplayer.interfaces.ICallbacks
import com.conamobile.conaplayer.interfaces.IMainActivityFragmentCallbacks
import com.conamobile.conaplayer.misc.UpdateToastMediaScannerCompletionListener
import com.conamobile.conaplayer.misc.WrappedAsyncTaskLoader
import com.conamobile.conaplayer.model.Song
import com.conamobile.conaplayer.providers.BlacklistStore
import com.conamobile.conaplayer.util.FileUtil
import com.conamobile.conaplayer.util.PreferenceUtil.startDirectory
import com.conamobile.conaplayer.util.ThemedFastScroller.create
import com.conamobile.conaplayer.util.getExternalStorageDirectory
import com.conamobile.conaplayer.util.getExternalStoragePublicDirectory
import com.conamobile.conaplayer.views.BreadCrumbLayout
...
class FoldersFragment : AbsMainActivityFragment(R.layout.fragment_folder),
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            switchToFileAdapter()
            setCrumb(BreadCrumbLayout.Crumb(FileUtil.safeGetCanonicalFile(startDirectory)), true)
        } else {
                ...
                when (val itemId = item.itemId) {
                    R.id.action_play_next, R.id.action_add_to_current_playing, R.id.action_add_to_playlist, R.id.action_delete_from_device -> {
                        lifecycleScope.launch(Dispatchers.IO) {
                            listSongs(requireContext(),
                                listOf(file),
                                AUDIO_FILE_FILTER,
                                fileComparator) { songs ->
                                if (songs.isNotEmpty()) {
                                    SongsMenuHelper.handleMenuClick(requireActivity(),
                                        songs,
                                        itemId)
                                }
                            }
                    ...
                    R.id.action_set_as_start_directory -> {
                        startDirectory = file
                        showToast(String.format(getString(R.string.new_start_directory), file.path))
                        return@setOnMenuItemClickListener true
                    }
                    R.id.action_scan -> {
                ...
                listSongs(requireContext(),
                    ...
                    if (songs.isNotEmpty()) {
                        ...
                        if (startIndex > -1) {
                            ...
                            Snackbar.make(mainActivity.slidingPanel,
                                String.format(getString(R.string.not_listed_in_media_store),
                                    mFile.name

                                ).parseAsHtml(),
                                Snackbar.LENGTH_LONG).setAction(R.string.action_scan) {
                                lifecycleScope.launch {
                                    listPaths(mFile, AUDIO_FILE_FILTER) { paths ->
                                        scanPaths(paths)
                                    }
                                }
                            }.setActionTextColor(accentColor(requireActivity())).show()
                        }
                    }
                }
            }
    ...
    override fun onCreateMenu(menu: Menu, inflater: MenuInflater) {
        ...
        ToolbarContentTintHelper.handleOnCreateOptionsMenu(requireContext(),
            toolbar,
            menu,
    ...
    override fun onMenuItemSelected(item: MenuItem): Boolean {
        when (item.itemId) {
            R.id.action_go_to_start_directory -> {
                setCrumb(BreadCrumbLayout.Crumb(tryGetCanonicalFile(startDirectory)), true)
                return true
            }
            R.id.action_scan -> {
    ...
    override fun onStorageClicked(storage: Storage) {
        switchToFileAdapter()
        setCrumb(BreadCrumbLayout.Crumb(FileUtil.safeGetCanonicalFile(storage.file)), true)
    }
    ...
    companion object {
        ...
        val AUDIO_FILE_FILTER = FileFilter { file: File ->
            (!file.isHidden && (file.isDirectory || FileUtil.fileIsMimeType(file,
                "audio/*",
                MimeTypeMap.getSingleton()) || FileUtil.fileIsMimeType(file,
                "application/opus",
                MimeTypeMap.getSingleton()) || FileUtil.fileIsMimeType(file,
                "application/ogg",
